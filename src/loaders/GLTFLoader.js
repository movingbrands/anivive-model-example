...
import {
  ...
  ALPHA_MODES,
  EXTENSIONS
} from './gltfLoaderConstants'

/**
 * GLTF file loader derived from {@link https://github.com/mrdoob/three.js/blob/dev/examples/js/loaders/GLTFLoader.js}
 * @class GLTFLoader
...
class GLTFLoader {
  /**
   * Creates a new loading manager
   * @param {LoadingManager} manager - (optional) loading manager
   **/
  constructor(manager) {
  ...
  /**
   * Load a GLTF file in .gltf (json) or .glb (binary)
   * @param {String} url - GLTF file url
   * @param {Function} onLoad - called once item has loaded
   * @param {Function} onProgress - called as loading progresses
   * @param {Function} onError - called if loading fails
   **/
  ...
  parse(data, path, onLoad, onError) {
    ...
    parser.parse((scene, scenes, asset) => {
      var glTF = {
        scene: scene,
        scenes: scenes,
        // cameras: cameras,
...
/**
 * Stores references to GLTF objects
 * @class GLTFRegistry
 * @private
 */
class GLTFRegistry {
...
function GLTFLightsExtension(json) {
  ...
  for (var lightId in lights) {
    ...
    if (lightNode) {
      ...
      lightNode.name = light.name || ['light_', lightId].join('')
...
function GLTFMaterialsPbrSpecularGlossinessExtension() {
  return {
    ...
    refreshUniforms: function (renderer, scene, geometry, material, group) {
 ...
 */
const addMorphTargets = (mesh, meshDef, primitiveDef, accessors) => {
...
const getCachedGeometry = (cache, newPrimitive) => {
 ...
 * @param  {Array<BufferAttribute>} accessors
 */
const addPrimitiveAttributes = (geometry, primitiveDef, accessors) => {
